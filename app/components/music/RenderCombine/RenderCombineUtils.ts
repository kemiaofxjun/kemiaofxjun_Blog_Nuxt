...
export function musicCardTransitionStart(
  ...
  // 开始动画时的状态回调
  stateCallbacks?.onTransitionStart?.();
  ...
  currentAnimation?.addEventListener("finish", () => {
    ...
    stateCallbacks?.onTransitionEnd?.();
    callback();
...
export function musicCardTransitionSliding(
  ...
  stateCallbacks?: {
    onTransitionStart?: () => void;
    onTransitionOvershoot?: (progress: number) => void;
  },
) {
  // 取消当前动画以实现跟手效果
  if (currentAnimation) {
  ...
  progress = Math.max(-1, Math.min(1, progress)); // 允许20%的负向过冲

  // 线性插值计算当前状态，支持负值进度的过冲效果
  // 不允许面板样式超出屏幕，只触发过冲效果回调函数
  const currentTop =
    fullScreenState.top + (miniCardState.top - fullScreenState.top) * progress <
    0
      ? 0
      : fullScreenState.top +
        (miniCardState.top - fullScreenState.top) * progress;
  const currentLeft =
  ...
  drawer.style.setProperty("top", currentTop + "px", "important");
  drawer.style.setProperty("left", currentLeft + "px", "important");
  drawer.style.setProperty("width", currentWidth + "px", "important");
  drawer.style.setProperty("height", currentHeight + "px", "important");
  drawer.style.setProperty(
    "border-radius",
    currentBorderRadius + "rem",
    "important",
  );
  ...
  if (progress < 0) {
    stateCallbacks?.onTransitionOvershoot?.(progress);
  }
}
...
export function musicCardTransitionSlideCancelDown(
  element: any,
  drawer: any,
  stateCallbacks?: {
    onTransitionEnd?: () => void;
